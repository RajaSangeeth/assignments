...
as_date(irl_house_core$death)
as_date(irl_house_core$birth)

## creating new variable and calcualitng age at death 
irl_house_core <- irl_house_core %>% 
              mutate(age_at_death = (death - birth)/365)

ggplot(irl_house_core)
...
# Bind together
top5_high_low <- as_tibble((bind_rows(top_5, bottom_5)))
```
...
irl_house_core %>% 
   mutate(name_lenght =as.character(name_lenght)) %>% 
   arrange(str_length(name_lenght), name_
